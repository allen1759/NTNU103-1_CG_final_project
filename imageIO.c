...
unsigned char* getDataForOpenGL(ColorImage *image) ;
void freeDataForOpenGl(unsigned char *);

unsigned char* getDataForOpenGL(ColorImage *image)
{
    int xSize = image->xRes, ySize = image->yRes;
    unsigned char * ptr = (unsigned char *)malloc(sizeof(unsigned char)*xSize*ySize*4);
    int i, j, ind = 0;
    for(i=0; i<ySize; i+=1) {
        for(j=0; j<xSize; j+=1) {
            ptr[ind++] = image->pPixel[i*ySize+j].B;
            ptr[ind++] = image->pPixel[i*ySize+j].G;
            ptr[ind++] = image->pPixel[i*ySize+j].R;
            ptr[ind++] = 255;
        }
    }
    return ptr;
}
void freeDataForOpenGl(unsigned char *image)
{
    free(image);
}
