...
//                else if(strncmp(buf, "map_Bump", 7) == 0)
//                {
//                    fscanf(file, "%s", buf);
//                    IMAGE texture;
//                    texture.LoadTGA(buf);
...
//                    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR_MIPMAP_LINEAR);
//                }
                else if(strncmp(buf, "map_Kd", 6) == 0)
                {
                    fscanf(file, "%s", buf);
                    char *check = buf;
                    while( *(check+1) != '\0' ) check+=1;

                    if( *check == 'a' ) {
                std::cout << "tga file name :  " << buf << std::endl;
                        IMAGE texture;
                        texture.LoadTGA(buf);
                        glGenTextures(1, &model->materials[nummaterials].textureID);
                        glBindTexture(GL_TEXTURE_2D, model->materials[nummaterials].textureID);
...
//                        glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T,GL_CLAMP);
                ...
                std::cout << "ppm file name :  " << buf << std::endl;
                        ColorImage texture;
                        glEnable(GL_TEXTURE_2D);
//                        readPPM("opengl.ppm", &texture);
                        readPPM(buf, &texture);
                        glGenTextures(1, &model->materials[nummaterials].textureID);
                        glBindTexture(GL_TEXTURE_2D, model->materials[nummaterials].textureID);
                        //glTexImage2D(GL_TEXTURE_2D,0,3,texture.xRes,texture.yRes,0,GL_RGB,GL_UNSIGNED_BYTE, texture.pPixel);

                        unsigned char * data = getDataForOpenGL(&texture);
                        gluBuild2DMipmaps(GL_TEXTURE_2D, 4, texture.xRes, texture.yRes, GL_BGRA, GL_UNSIGNED_BYTE, data);
                        freeDataForOpenGl(data);
